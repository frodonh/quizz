...
function shuffle_once($arr) {
	...
	for ($i=0;$i<$num;$i++) {
		...
		while ($pair!==false) {
			...
			$newpair=$newpair.strval($arr[$pair]['id']);
			$res[$i]=$newpair;
			$pair=strtok(',');
		}
	}
...
if (array_key_exists('action',$_POST))	{	// Action sans paramètre
	if ($_POST['action']=="list") {	// Liste des agents
		...
		$query="select id,nom,prenom from seminaire.agents order by nom,prenom";
	...
	elseif ($_POST['action']=="view_pairs") {	// Liste des agents
		$dbconn=pg_connect("host=".$host." dbname=".$dbname." user=".$user." password=".$password) or die ('Impossible to connect to the database: '.pg_last_error());
		$query="select concat(a.nom,' ',a.prenom) as first,string_agg(concat(b.nom,' ',b.prenom),',') as second from seminaire.agents a inner join seminaire.agents b on cast(b.id as text) in (select regexp_split_to_table(a.paire,',')) group by first";
		$res=pg_query($query) or die('Request failed: '.pg_last_error());
		$val=array();
		while ($row=pg_fetch_row($res)) array_push($val,$row);
		header("Content-Type: application/json");
	...
	elseif ($_POST['action']=="shuffle") { // Génère les paires
		...
		for ($i=0;$i<count($val);$i++) {
			pg_query_params("update seminaire.agents set paire=$1 where id=$2",array($best[0][$i],$val[$i]['id'])) or die('Request failed: '.pg_last_error());
		}
		echo "Score : {$best[1]}";
		pg_close($dbconn);
	}
...
elseif (array_key_exists('id',$_POST)) { // Details
	$dbconn=pg_connect("host=".$host." dbname=".$dbname." user=".$user." password=".$password) or die ('Impossible to connect to the database: '.pg_last_error());
	$res=pg_query_params("select * from seminaire.agents where id=$1",array($_POST['id'])) or die('Request failed: '.pg_last_error());
...
elseif (array_key_exists('register',$_POST)) { // Enregistrement
	$dbconn=pg_connect("host=".$host." dbname=".$dbname." user=".$user." password=".$password) or die ('Impossible to connect to the database: '.pg_last_error());
	$res=pg_query_params("select id from seminaire.agents where concat(nom,' ',prenom)=$1",array($_POST['register'])) or die('Request failed: '.pg_last_error());
	...
	$res=pg_query_params("update seminaire.agents set present=true where id=$1",array($id)) or die('Request failed: '.pg_last_error());
	pg_free_result($res);
	echo $id;
	pg_close($dbconn);
}
elseif (array_key_exists('join',$_POST)) { // Création
	$dbconn=pg_connect("host=".$host." dbname=".$dbname." user=".$user." password=".$password) or die ('Impossible to connect to the database: '.pg_last_error());
	$res=pg_query_params("insert into seminaire.agents (nom,prenom,direction,service,unite,fonction,site,present) values ($1,$2,'DSUED',$3,$4,$5,$6,TRUE) returning id",array($_POST['name'],$_POST['fname'],$_POST['service'],$_POST['unit'],$_POST['function'],$_POST['location'])) or die('Request failed: '.pg_last_error());
	$val=pg_fetch_row($res);
	pg_free_result($res);
	if (!$val) {
		echo "KO";
...
elseif (array_key_exists('pair',$_POST)) { // Retour de la paire
	$dbconn=pg_connect("host=".$host." dbname=".$dbname." user=".$user." password=".$password) or die ('Impossible to connect to the database: '.pg_last_error());
	$res=pg_query_params("select * from seminaire.agents where id=(select paire from seminaire.agents where id=$1)",array($_POST['pair'])) or die('Request failed: '.pg_last_error());
	$val=pg_fetch_array($res,null,PGSQL_ASSOC);
	header("Content-Type: application/json");
	echo json_encode($val);
	pg_free_result($res);
